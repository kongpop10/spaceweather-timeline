...
if not XAI_API_KEY:
    try:
        ...
        with open(".streamlit/secrets.toml", "r") as f:
            secrets = toml.load(f)
            XAI_API_KEY = secrets.get("XAI_API_KEY", "")
    except Exception as e:
        logger.warning(f"Could not load API key from secrets.toml: {e}")
...
def call_llm(prompt, provider="grok"):
    ...
    try:
        ...
        if provider == "grok":
            ...
            # Check if API key is available
            if not api_key:
                logger.error("No API key available for Grok. Set the XAI_API_KEY environment variable or add it to .streamlit/secrets.toml")
                return None
        else:
...
def main():
    ...
    if not XAI_API_KEY:
        logger.error("No API key available for Grok. Set the XAI_API_KEY environment variable or add it to .streamlit/secrets.toml")
        print("ERROR: No API key available. Please set the XAI_API_KEY environment variable or add it to .streamlit/secrets.toml")
        return
